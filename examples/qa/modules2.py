...
class DataFilter(nn.Module):
    ...
    def pad_add_special_tokens(self, tensor):
    ...
    def get_cut_input(self, input_ids, shift_batch):
        B, T = input_ids.shape
        ...
        for i, seq in enumerate(input_ids):
            context = seq[(seq != self.pad_token_id) & (seq != self.cls_token_id) & (seq != self.sep_token_id)]  # context

            start = shift_batch[i].item()
            end = max(start, start + self.segment_size - 2)
            end = min(end, len(context))
            input_segment = context[start:end] 
            input_segment = self.pad_add_special_tokens(input_segment)
    ...
    def forward(self, batch: Dict[str, Tensor], state: State = None, info: Info = {}, *args) -> Tuple[Dict[str, Tensor], Done]:
        ...
        if "shift_batch" not in info:
            info["shift_batch"] = torch.zeros(batch['input_ids'].shape[0], dtype=torch.int32)
        
        input_ids, new_shift = self.get_cut_input(batch['input_ids'], info["shift_batch"])
...
class Predictor(nn.Module):
    ...
    def __init__(self, bert_config):
        ...
        config2.hidden_dropout_prob = 0.15
...
class RobertaRT(nn.Module):
    ...
    def __init__(self, roberta: RobertaModel):
        ...
        self.bert = roberta
        self.bert.config.hidden_dropout_prob = 0.15
        self.bert.train()
