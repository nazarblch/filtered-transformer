...
weights = torch.load("/home/buzun/enformer.pt")
...
train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=20, pin_memory=True, num_workers=5, collate_fn=collate_fn)
...
writer = SummaryWriter("/home/buzun/pomoika/enformer2.0")
...
for it, batch in enumerate(train_dataloader):
    ...
    print()
    state = torch.zeros(batch["center"]["labels"].shape[0], 200, model_cfg.hidden_size, device=torch.device("cuda:1"))
    ...
    with torch.no_grad():
        ...
        for j in range(0, 896, 14):
            pred_j = pred_acc(context[:, j:j+14].cuda(), last_state).cpu()
