...
tr_dim = 256
filter_model = ChunkFilter(T22(tr_dim, 4, 3, 3, 512, batch_first=True, dropout=0), 10, 2, tr_dim, 2).cuda()
...
#         sample_size=10
#     )
f_tr = FilteredTransformer(T22(tr_dim, 4, 3, 3, 512, batch_first=True, dropout=0), filter_model, 2).cuda()
...
writer = SummaryWriter(f"/tmp/nazar/pomoika/add_{time.time()}")
